...
## Owning code
`harness/config_engine.py` — `ConfigLoader.load_and_validate()`.
...
(`harness/schemas.py:25`) — or a fail-closed error if resolution can't
complete.
...
Then config loading FAILS with a `ValueError` naming the missing bundle — fails closed, no partial/default config is promoted (`harness/config_engine.py:51-58`)
...
Then config loading FAILS — `agent_id` must map to exactly one BU, never shared, since OpenClaw-style isolation depends on this (`harness/config_engine.py:60-75`)
...
When `_validate_uniqueness()` runs
Then config loading PASSES — one BU reachable via multiple channels is explicitly fine (`harness/config_engine.py:61-64`'s docstring)

## Status
**Real, tested.** `harness/config_engine.py` implements exactly this;
see `tests/test_harness.py` for proof. This is one of the two most-built
