...
- [x] 2.4 Tests: app-scoped requester denied; foundation-scoped and both-scoped requesters pass

## 3. `FoundationRecord`

- [x] 3.1 Add `FoundationRecord` to `gateway/schemas.py`, including `compute_paradigm: str = "kubernetes"` per design.md
- [x] 3.2 Add the `foundation_records` SQLite table, same `db_path` as `BrokeredToolDispatcher`
- [x] 3.3 Add a small store (`record_foundation()`, `get_foundation()`) — mirrors `SkillUsageStore`'s shape
- [x] 3.4 Tests: write and read a `FoundationRecord`, confirm `compute_paradigm`/`layer` defaults
...
## 5. `gateway/kubernetes_foundation_dispatch.py`
...
- [x] 5.7 On success, write the `FoundationRecord` (task 3.3's store)
- [x] 5.8 Define `ClusterDispatchResult` (Pydantic: status, resource_identifier, error_message)

## 6. Tests

- [x] 6.1 Test: denied `ToolIntent` (human_approved=False) never calls any MCP tool, no `FoundationRecord` written
- [x] 6.2 Test: AWS path — mocked `manage_eks_stacks` success writes a `FoundationRecord` with `cloud_provider="aws"`, `compute_paradigm="kubernetes"`
...
- [x] 6.6 Test: a failed execution (mocked MCP error) does not write a `FoundationRecord`
- [x] 6.7 Run the full test suite (`pytest tests/`) and confirm no regressions — 80 passed, 0 failed

## 7. Manual real-credential checklist (not automated — for whoever runs this with real cloud access)

- [ ] 7.1 AWS: run the flow end to end against a real sandbox account, confirm a real EKS cluster is created and `FoundationRecord` matches it
