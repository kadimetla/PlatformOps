...
| 14 | Optional, after 13: resource-primitive authoring — per-provider resource registries (exact discriminated config schemas + reviewed renderers), immutable `TopologyRevision` chain, coding-agent architecture review plus compose/repair loop bounded at 2 automatic rounds, revision-scoped unit/artifact state, and seal/supersede lifecycle; Deep Agents is a separately accepted runtime implementation option and the stronger candidate for sandboxed Level 2 module/renderer authoring | Unknown resource types and out-of-schema configurations fail closed; LLM output is typed resource data only, never HCL; every changed node/edge/config creates a successor revision and atomically invalidates compiled results/artifact/plan/policy/approval; a runtime Deep Agent has no filesystem/shell/default subagent/skills/memory/store/checkpointer and exposes only scoped read-only tools; an offline authoring Deep Agent runs in an isolated credential-free sandbox and exports reviewable PR artifacts only; approval binds one exact revision | Pending, strictest checks of any composition level — runtime and offline Deep Agent acceptance decisions remain independent; see `COMPOSABLE_PROVISIONER.md`'s Deep Agents, resource-authoring, and fluid-lifecycle sections |
