...
| **What would be PlatformOps's actual analog to OpenClaw's harness** | — | Not built. Closest fit: `PROVISION_WORKFLOW.md`'s toolchain selection (`ccapi`/`hcp_terraform`/`opentofu_local`/`terraform_local`, selected from trusted workspace ownership—not whichever binary happens to be available) — a swappable single-step executor with engine/version sealed for local plans, exactly OpenClaw's harness shape |
