...
| [PROVISION_WORKFLOW.md](PROVISION_WORKFLOW.md) | New-stack vs. existing-stack provisioning: identical access flow for both, the CCAPI-vs-Terraform diffing asymmetry (Terraform gets a plan engine free, CCAPI needs a custom diff builder), concurrent-drift protection (`current_state_fingerprint` extending `approval_digest`), action-verb allow-list extension for gating delete separately from create/update, template-first IaC generation (Level 1 render / Level 2 template-library PR / Level 3 executor — never freshly-generated unreviewed IaC), corrected Terraform execution model (HCP managed runs, not local CLI) | Designed only; CCAPI's lack of a native diff primitive stated as believed-true, flagged verify-before-build; corrects `skills/provision-infra/SKILL.md`'s free-form-drafting step, not yet applied there |
