"""The stack-scope gate -- openspec/changes/provision-kubernetes-cluster.
Renamed from "foundation" scope (docs/composable_foundation_blueprints.md
Parts G/M -- "no more foundation/platform," matches Stack/Resource
terminology). A structural check, not a prompt instruction: denies a
cluster-creation request before any skill/tool resolution runs if the
requester's TeamMember.scope doesn't include "stack". Matches
...
def requester_has_stack_scope(bundle: WorkspaceBundle, channel_user_id: str) -> bool:
    ...
    for member in bundle.members:
        if member.channel_user_id == channel_user_id:
            return member.scope in ("stack", "both")
    return False
